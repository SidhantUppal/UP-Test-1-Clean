...
interface AnimatedIncidentsByTypeWidgetProps {
  id?: string;
  isEditMode?: boolean;
}

export default function AnimatedIncidentsByTypeWidget({ id, isEditMode }: AnimatedIncidentsByTypeWidgetProps) {
  ...
  const options = {
    ...
    animation: {
      duration: 0, // Disable Chart.js animation since we're handling it ourselves
    },
    plugins: {
      legend: {
    ...
    scales: {
      ...
      y: {
        beginAtZero: true,
        max: Math.max(...targetData.thisMonth, ...targetData.lastMonth) + 5,
        grid: {
          color: '#f3f4f6',
        },
      },
  ...
  const animatedTotal = Math.floor(animatedData.thisMonth.reduce((a, b) => a + b, 0));
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <div className="text-2xl font-bold text-gray-800 transition-all duration-1500">
              {animatedTotal}
            </div>
        ...
        <div className={`grid grid-cols-2 gap-3 text-center transition-all duration-700 
