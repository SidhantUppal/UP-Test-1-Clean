...
interface EnvironmentalParticipationWidgetProps {
  id: string;
  isEditMode: boolean;
}

export default function EnvironmentalParticipationWidget({ id, isEditMode }: EnvironmentalParticipationWidgetProps) {
  const participationData = {
    totalUsers: 125,
    activeToday: 105,
    participationRate: 84,
    departments: [
      { name: 'Production', rate: 88, users: '35/40' },
      { name: 'Warehouse', rate: 82, users: '25/30' },
      { name: 'Maintenance', rate: 75, users: '15/20' },
      { name: 'Quality', rate: 95, users: '19/20' },
      { name: 'Admin', rate: 73, users: '11/15' }
    ],
    weeklyTrend: '+8%'
  };
