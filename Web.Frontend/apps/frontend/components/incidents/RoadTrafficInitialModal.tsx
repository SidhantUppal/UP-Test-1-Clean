...
export interface RoadTrafficInitialData {
  reference: string;
...
interface RoadTrafficInitialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveAndContinue: (data: RoadTrafficInitialData & { incidentCaseId: number }) => void;
}

export const RoadTrafficInitialModal: React.FC<RoadTrafficInitialModalProps> = ({
  isOpen,
  onClose,
  onSaveAndContinue
}) => {
  ...
  const [formData, setFormData] = useState<RoadTrafficInitialData>(() => {
  ...
  const handleFieldChange = (field: keyof RoadTrafficInitialData, value: string | number | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call backend to create initial incident
      const response = await incidentService.createInitialRoadTrafficIncident({
      ...
      // Call parent callback with both form data and incident case ID
      onSaveAndContinue({
        ...formData,
        incidentCaseId: response.IncidentCaseID, // Use PascalCase property from API
        reference: response.CaseNumber // Use the generated case number (PascalCase)
      });
    } catch (error) {
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-semibold text-gray-900" id="modal-title">
                New Road Traffic Incident Report Form
              </h3>
              <button
                type="button"
...
export default RoadTrafficInitialModal;