...
export interface HighPotentialInitialData {
  reference: string;
...
interface HighPotentialInitialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveAndContinue: (data: HighPotentialInitialData & { incidentCaseId: number }) => void;
}

export const HighPotentialInitialModal: React.FC<HighPotentialInitialModalProps> = ({
  isOpen,
  onClose,
  onSaveAndContinue
}) => {
  ...
  const [formData, setFormData] = useState<HighPotentialInitialData>(() => {
  ...
  const handleFieldChange = (field: keyof HighPotentialInitialData, value: string | number | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await incidentService.createInitialHighPotentialIncident({
...
export default HighPotentialInitialModal;