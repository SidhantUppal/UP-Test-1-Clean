...
export interface NearMissInitialData {
  reference: string;
...
interface NearMissInitialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveAndContinue: (data: NearMissInitialData & { incidentCaseId: number }) => void;
}

export const NearMissInitialModal: React.FC<NearMissInitialModalProps> = ({
  isOpen,
  onClose,
  onSaveAndContinue
}) => {
  ...
  const [formData, setFormData] = useState<NearMissInitialData>(() => {
  ...
  const handleFieldChange = (field: keyof NearMissInitialData, value: string | number | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.severity) {
      setError('Please select a potential severity level');
      return;
    }
    ...
    try {
      console.log('Submitting Near Miss modal data:', formData);
      
      // Call backend API to create initial Near Miss incident
      const result = await incidentService.createInitialNearMissIncident({
      ...
      console.log('Near Miss incident created:', result);
      ...
      console.error('Error creating Near Miss incident:', err);
      setError(err.message || 'Failed to save Near Miss incident. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Potential severity of incident <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <select
          ...
          <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row sm:px-6 gap-3">
            ...
            <button
              ...
              disabled={isLoading}
              style={{ 
                backgroundColor: isLoading ? '#9ca3af' : accentColor, 
                color: '#ffffff', 
                border: 'none',
                padding: '8px 16px',
                borderRadius: '6px',
...
export default NearMissInitialModal;