...
import { Hazard, calculateSeverityLevel, getSeverityColor } from '@/types/hazard.types';
...
export default function HazardReportsPage()
{
    ...
    const loadHazards = async () =>
    {
        try
        {
            ...
            const mappedHazards = hazardData.map((h: any) => ({
                ...
                categoryName: h.Title || h.categoryName,
                categoryTypeName: h.Title || h.categoryTypeName,
                ...
                ...calculateSeverityLevel(h.InherentRiskScore || h.inherentRiskScore)
            }));
    ...
    const metrics = useMemo(() =>
    {
        ...
        const highRiskCount = hazards.filter(h => h.SeverityTypeName === 'Major' || h.SeverityTypeName === 'Catastrophic').length;
        ...
        hazards.forEach(h =>
        {
            const category = h.Title || 'Uncategorized';
        ...
        hazards.forEach(h =>
        {
            if (h.SeverityTypeName)
            {
                severityMap.set(h.SeverityTypeName, (severityMap.get(h.SeverityTypeName) || 0) + 1);
            }
        });
    ...
    const treemapData = useMemo(() =>
    {
        ...
        metrics.categoryMap.forEach((count, category) =>
        {
            ...
            const categoryHazards = hazards.filter(h => (h.Title || 'Uncategorized') === category);
                    ...
                    <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                        {['overview', 'trends', 'completion', 'details'].map((view) => (
                            <a
                                ...
                                className={`py-4 px-1 border-b-2 font-medium text-sm capitalize ${selectedView === view
            ...
            <div className="px-8 py-8 lg:px-12 xl:px-16 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
                        ...
                        <p className="text-3xl font-bold" style={{ color: '#3d3a72' }}>{metrics.activeHazards}</p>
                        <p className="text-sm text-gray-500 mt-1">Requires action</p>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
                        <h3 className="text-lg font-semibold mb-2">Resolved</h3>
                        <p className="text-3xl font-bold" style={{ color: '#3d3a72' }}>{metrics.resolvedHazards}</p>
                        <p className="text-sm text-gray-500 mt-1">Completed</p>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
                        <h3 className="text-lg font-semibold mb-2">High Risk</h3>
                        <p className="text-3xl font-bold" style={{ color: '#3d3a72' }}>{metrics.highRiskCount}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
                        ...
                        <p className="text-3xl font-bold" style={{ color: '#3d3a72' }}>{metrics.avgResolutionTime}</p>
                                        ...
                                        <table className="w-full">
                                            <thead className="bg-purple-50">
                                                <tr>
                                                    ...
                                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Hazards</th>
                                            ...
                                            <tbody className="divide-y divide-gray-200">
                                                {Array.from(metrics.locationMap.entries())
                                                    ...
                                                    .map(([location, count]) =>
                                                    {
                                                        ...
                                                        return (
                                                            <tr key={location} className="hover:bg-purple-50">
                                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{location}</td>
                                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-right">{count}</td>
                                        ...
                                        <table className="w-full">
                                            <thead className="bg-purple-50">
                                                <tr>
                                                    ...
                                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Avg Days</th>
                                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Priority</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-200">
                    ...
                    <>
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {
                                    const total = hazards.filter(h => h.SeverityTypeName === severity).length;
                                    const resolved = hazards.filter(h => h.SeverityTypeName === severity && !h.IsActive).length;
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex justify-between py-2">
                                        ...
                                        <span className="font-medium text-red-600">
                                            {hazards.filter(h => h.IsActive && h.SeverityTypeName === 'Catastrophic').length}
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        {topHazards.map((hazard) => (
                                            <tr key={hazard.HazardID} className="hover:bg-purple-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{hazard.Title}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{hazard.LocationName || '-'}</td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span
                                                        className="inline-flex items-center px-2 py-1 rounded text-xs font-medium"
                                                        style={{
                                                            backgroundColor: getSeverityColor(hazard.SeverityTypeName || '') + '20',
                                                            color: getSeverityColor(hazard.SeverityTypeName || '')
                                                        }}
                                                    >
                                                        {hazard.SeverityTypeName}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    {hazard.IsActive ? (
