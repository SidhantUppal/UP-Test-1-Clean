...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${STATIC_DATA_API_URL}/api/StaticData/hazard-category-types`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Static Data Hazard Categories API Error Status:', response.status);
      console.error('Static Data Hazard Categories API Error Text:', errorText);
    ...
    console.error('=== HAZARD CATEGORY TYPES API ROUTE ERROR ===');
    console.error('Error proxying request to Static Data API:', error);
    return NextResponse.json(
      { error: 'Failed to fetch hazard category types' },
      { status: 500 }
    );
  }
}
