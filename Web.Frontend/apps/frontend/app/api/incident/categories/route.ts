...
// Use Static Data API for incident categories
...
// GET /api/incident/categories - Get incident categories
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      // Try static data API for incident categories
      const response = await fetch(
        `${STATIC_DATA_SERVICE_URL}/api/StaticData/incident-category-types${queryString ? `?${queryString}` : ''}`,
        {
          method: 'GET',
          headers: {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch incident categories' },
      { status: 500 }
    );
  }
}

// POST /api/incident/categories - Create incident category
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${INCIDENTS_SERVICE_URL}/api/incident/categories`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': cookies, // Forward cookies to backend
    ...
    return NextResponse.json(
      { error: 'Failed to create incident category' },
      { status: 500 }
    );
  }
}